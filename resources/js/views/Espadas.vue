<template>
  <v-container class="pa-6">
    <v-row class="align-center justify-space-between mb-4">
      <h1 class="text-h5 font-weight-bold">Cadastro de Espadas</h1>
      <v-btn color="primary" prepend-icon="mdi-plus" @click="openDialog">Nova Espada</v-btn>
    </v-row>

    <v-data-table :headers="headers" :items="items" :loading="loading" class="elevation-2" item-value="id" no-data-text="Nenhuma espada cadastrada">
    ...
    <v-dialog v-model="dialog" persistent max-width="500px">
      <v-card>
        <v-card-title class="text-h6 font-weight-bold">{{ editMode ? 'Editar Espada' : 'Nova Espada' }}</v-card-title>
        <v-card-text>
    ...
    <v-dialog v-model="confirmDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text>Tem certeza que deseja excluir a espada <strong>#{{ selected?.id }}</strong>?</v-card-text>
        <v-card-actions>
          <v-spacer />
...
const fetchItems = async () => {
  loading.value = true
  try { const { data } = await api.get('/espadas'); items.value = data } catch (err) { console.error('Erro ao carregar espadas', err) } finally { loading.value = false }
}
...
const saveItem = async () => {
  ...
  if (!isValid) return
  try {
    if (editMode.value) await api.put(`/espadas/${form.value.id}`, form.value)
    else await api.post('/espadas', form.value)
    await fetchItems(); dialog.value = false
  } catch (err) { console.error('Erro ao salvar espada', err.response?.data || err) }
}
...
const deleteItem = async () => { try { await api.delete(`/espadas/${selected.value.id}`); confirmDialog.value = false; await fetchItems() } catch (err) { console.error('Erro ao excluir espada', err) } }
