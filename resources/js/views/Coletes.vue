<template>
  <v-container class="pa-6">
    <v-row class="align-center justify-space-between mb-4">
      <h1 class="text-h5 font-weight-bold">Cadastro de Coletes</h1>
    ...
    <v-data-table
      ...
      class="elevation-2"
      item-value="id"
      no-data-text="Nenhum colete cadastrado"
    >
      <template #item.acoes="{ item }">
        <v-btn icon size="small" color="primary" variant="text" @click="editItem(item)">
    ...
    <v-dialog v-model="dialog" persistent max-width="500px">
      <v-card>
        <v-card-title class="text-h6 font-weight-bold">{{ editMode ? 'Editar Colete' : 'Novo Colete' }}</v-card-title>
        <v-card-text>
    ...
    <v-dialog v-model="confirmDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text>Tem certeza que deseja excluir o colete <strong>#{{ selected?.id }}</strong>?</v-card-text>
        <v-card-actions>
          <v-spacer />
...
const fetchItems = async () => {
  loading.value = true
  try {
    const { data } = await api.get('/coletes')
    items.value = data
  } catch (err) {
    console.error('Erro ao carregar coletes', err)
  } finally {
    loading.value = false
  }
}
...
const saveItem = async () => {
  ...
  try {
    if (editMode.value) {
      await api.put(`/coletes/${form.value.id}`, form.value)
    } else {
      await api.post('/coletes', form.value)
    }
    await fetchItems()
    dialog.value = false
  } catch (err) { console.error('Erro ao salvar colete', err.response?.data || err) }
}
...
const deleteItem = async () => {
  try {
    await api.delete(`/coletes/${selected.value.id}`)
    confirmDialog.value = false
    await fetchItems()
  } catch (err) { console.error('Erro ao excluir colete', err) }
}
